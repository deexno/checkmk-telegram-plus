...
You can also use `notifications_smart` as the first parameter. Smart notifications send the full Checkmk alert to the bot, enrich it with live Checkmk parent/child dependency context and recent SQLite notification history, use the OpenAI Responses API, and then decide whether the alert should be sent loud, sent silent, or suppressed. Create this rule broadly in Checkmk if you want the bot to filter duplicates, flapping, likely follow-up failures and relevant recoveries itself. Users can subscribe to Smart notifications in Telegram under notification settings. Configure the OpenAI token in `/etc/checkmk-telegram-plus/<omd_site_name>.ini`; customize the policy in the web admin config page or in `/etc/checkmk-telegram-plus/smart-notification-instructions.txt`.<br><br>
...
notifications_smart = XXXXXXXXXXXXXX
